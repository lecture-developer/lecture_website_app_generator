...
class GitHubAutoWebsiteDeploy {
  constructor(
    loginMethod = githubLoginType.TOKEN,
    loginDetails,
    is_new_website,
    isNewWebsite,
    pathsWithFileContent
  ) {
  ...
  auth(loginMethod) {
    ...
    } // (loginMethod == githubLoginType.CRADENTIOALS)
    else {
      this.thisclient = github.client({
