...
def generate_registration_email(id: str,
                                email: str,
                                name: str,
                                token: str):
    EmailSenderGmail.send_email_via_gmail_with_html(to=email,
                                                    subject="Hi {}, please verify your Lecture Website App Generator account".format(name),
                                                    text="<h1>Hi, {}</h1><p>Thanks for using Lecture Website App Generator!Please confirm your email address by clicking on the link below:</p><p>{}/users/activation?token={}&user={}</p><p>Happy generating,The Lecture Website App Generator team</p>`".format(name, DOMAIN, token, id))


def generate_db_connection_failed_email(err: str):
    EmailSenderGmail.send_email_via_gmail_with_html(to=None,
                                                    subject="DB connection failed",
                                                    text="DB connection failed with error: {}".format(err))


def generate_forgot_password_email(email: str,
                                   token: str,
                                   user_id: str):
    # TODO: fix here later
    EmailSenderGmail.send_email_via_gmail_with_html(to=email,
                                                    subject="Forgot your password?",
                                                    text="{}/users/forget_password?token={}&user={}".format(DOMAIN, token, user_id))


def generate_new_not_activated_email(email: str,
                                     name: str,
                                     token: str):
    # TODO: fix here later
    EmailSenderGmail.send_email_via_gmail_with_html(to=email,
...
def generate_user_signed_for_beta_email(name: str,
                                        email: str,
                                        institution: str,
                                        research: str):
    EmailSenderGmail.send_email_via_gmail_with_html(to=None,
...
def generate_signed_for_beta_feedback(name: str,
                                      email: str):
    EmailSenderGmail.send_email_via_gmail_with_html(to=email,
...
def generate_user_joins_our_team_email(name: str,
                                       phone: str,
                                       email: str,
                                       short_bio: str):
    EmailSenderGmail.send_email_via_gmail_with_html(to=None,
...
def generate_user_contacts_us_email(name: str,
                                    email: str,
                                    message: str):
    EmailSenderGmail.send_email_via_gmail_with_html(to=None,
